...
impl<D> MotorElectric<D>
where
    D: DrawTarget<Color = Gray4>,
{
    pub fn new(position: Point) -> Self {
        let motor_bmp = MOTOR_BMP.init(
            Bmp::from_slice(include_bytes!("../../../assets/motor_electric_body.bmp")).unwrap(),
        );
        let motor_on_bmp = MOTOR_ON_BMP.init(
            Bmp::from_slice(include_bytes!(
                "../../../assets/motor_electric_symbol_on.bmp"
            ))
            .unwrap(),
        );
        let motor_off_bmp = MOTOR_OFF_BMP.init(
            Bmp::from_slice(include_bytes!(
                "../../../assets/motor_electric_symbol_off.bmp"
            ))
            .unwrap(),
        );
