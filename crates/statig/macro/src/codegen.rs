...
use quote::quote;
use syn::{parse_quote, Arm, GenericParam, ItemEnum, ItemFn, ItemImpl, Lifetime, LifetimeDef, Variant};

use crate::{
    lower::{Ir, Mode},
    CONTEXT_LIFETIME, EVENT_LIFETIME, SUPERSTATE_LIFETIME,
};
...
fn codegen_state_machine_impl(ir: &Ir) -> ItemImpl {
    ...
    let (impl_generics, _, where_clause) = &ir.state_machine.shared_storage_generics.split_for_impl();
...
fn codegen_state_impl(ir: &Ir) -> ItemImpl {
    ...
    let constructors: Vec<ItemFn> = ir.states.values().map(|state| &state.constructor).cloned().collect();
    ...
    match ir.state_machine.mode {
        ...
        Mode::Awaitable => parse_quote!(
            ...
            impl #impl_generics statig::awaitable::State<#shared_storage_type> for #state_ident #state_generics #where_clause
            {
                ...
                ) -> core::pin::Pin<statig::Box<dyn core::future::Future<Output = statig::Response<Self>> + 'fut>> {
...
fn codegen_superstate(ir: &Ir) -> ItemEnum {
    ...
    let variants: Vec<Variant> = ir.superstates.values().map(|superstate| superstate.variant.clone()).collect();
    let visibility = &ir.state_machine.visibility;
...
fn codegen_superstate_impl_superstate(ir: &Ir) -> ItemImpl {
    ...
    shared_storage_generics.params.push(superstate_lifetime_param);
            ...
            parse_quote!(
                ...
                impl #impl_generics statig::awaitable::Superstate<#shared_storage_type> for #superstate_ident #superstate_generics #where_clause
                {
                    fn call_handler<'fut>(
                        ...
                        #event_ident: &'fut <#shared_storage_type as statig::IntoStateMachine>::Event<'_>,
                        #context_ident: &'fut mut <#shared_storage_type as statig::IntoStateMachine>::Context<'_>
                    ) -> core::pin::Pin<statig::Box<dyn core::future::Future<Output = statig::Response<<#shared_storage_type as statig::IntoStateMachine>::State>> + 'fut>> {
                        statig::Box::pin(async move {
                            match self {
                                #(#call_handler_arms),*
                            }
