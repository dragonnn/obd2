...
use crate::{Box, IntoStateMachine, Response, StateOrSuperstate};
    ...
    for<'sub> M::Superstate<'sub>: Superstate<M>,
{
    ...
    fn common_ancestor_depth(mut source: M::Superstate<'_>, mut target: M::Superstate<'_>) -> usize {
        ...
        Box::pin(async move {
            ...
            match response {
                ...
                Response::Super => match self.superstate() {
                    Some(mut superstate) => {
                        M::ON_DISPATCH(shared_storage, StateOrSuperstate::Superstate(&superstate), event);
    ...
    for<'ctx> M::Context<'ctx>: Sync,
{
    ...
    fn call_entry_action(&mut self, _: &mut M, _: &mut M::Context<'_>) -> Pin<Box<dyn Future<Output = ()>>> {
        Box::pin(core::future::ready(()))
    }

    fn call_exit_action(&mut self, _: &mut M, _: &mut M::Context<'_>) -> Pin<Box<dyn Future<Output = ()>>> {
        Box::pin(core::future::ready(()))
    }
