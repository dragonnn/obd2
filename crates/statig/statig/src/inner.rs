...
use crate::{
    blocking::{self, StateExt as _},
    IntoStateMachine, Response,
};
    ...
    for<'sub> M::Superstate<'sub>: blocking::Superstate<M>,
{
    ...
    pub fn init_with_context(&mut self, context: &mut M::Context<'_>) {
        ...
        self.state.enter(&mut self.shared_storage, context, enter_levels);
    }
    ...
    pub fn transition(&mut self, mut target: M::State, context: &mut M::Context<'_>) {
        ...
        self.state.exit(&mut self.shared_storage, context, exit_levels);
...
where
    M: IntoStateMachine,
    for<'evt> M::Event<'evt>: Sync,
    for<'ctx> M::Context<'ctx>: Sync,
    ...
    for<'sub> M::Superstate<'sub>: awaitable::Superstate<M>,
{
    pub async fn async_init_with_context(&mut self, context: &mut M::Context<'_>) {
        let enter_levels = self.state.depth();
        self.state.enter(&mut self.shared_storage, context, enter_levels).await;
    }

    pub async fn async_handle_with_context(&mut self, event: &M::Event<'_>, context: &mut M::Context<'_>) {
        let response = self.state.handle(&mut self.shared_storage, event, context).await;
        match response {
            Response::Super => {}
            Response::Handled => {}
    ...
    pub async fn async_transition(&mut self, mut target: M::State, context: &mut M::Context<'_>) {
        ...
        // Perform the exit from the previous state towards the common ancestor state.
        self.state.exit(&mut self.shared_storage, context, exit_levels).await;
...
{
    fn clone(&self) -> Self {
        Self { shared_storage: self.shared_storage.clone(), state: self.state.clone() }
    }
}
    ...
    M::State: serde::Deserialize<'de>,
{
        ...
        D: serde::Deserializer<'de>,
    {
            ...
            M::State: serde::Deserialize<'de>,
        {
            ...
            {
                let shared_storage = seq.next_element()?.ok_or_else(|| serde::de::Error::invalid_length(0, &self))?;
                let state = seq.next_element()?.ok_or_else(|| serde::de::Error::invalid_length(1, &self))?;
                let inner = Inner { shared_storage, state };
                Ok(inner)
            }
                ...
                V: serde::de::MapAccess<'de>,
            {
                ...
                let shared_storage = shared_storage.ok_or_else(|| serde::de::Error::missing_field("shared_storage"))?;
        ...
        deserializer.deserialize_struct("StateMachine", FIELDS, InnerVisitor(PhantomData::default()))
    }
}
