...
where
    for<'sub> Self::Superstate<'sub>: awaitable::Superstate<Self>,
    Self::State: awaitable::State<Self>,
{
    ...
    where
        Self: Sized,
    {
        let inner = Inner { shared_storage: self, state: Self::INITIAL };
        StateMachine { inner, initialized: false }
    }
...
{
    fn default() -> Self {
        let inner = Inner { shared_storage: M::default(), state: M::INITIAL };
    ...
    M::State: serde::Deserialize<'de>,
{
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        Ok(StateMachine { inner, initialized: false })
    }
}
