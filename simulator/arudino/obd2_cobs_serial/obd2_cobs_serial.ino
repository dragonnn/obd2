...
void responseDataOBD(unsigned char data[], int size) {
  ...
  Serial.println("end sending");*/
  BuildMessage = ""; 
  Serial.print("T: ");Serial.print(canMsgOutgoing.can_id);Serial.print(",");
 ...
 void setup() {
  ...
  mcp2515v2.reset();
  mcp2515v2.enableOSM();
  mcp2515v2.setBitrate(CAN_500KBPS, MCP_8MHZ); // Your vehicle may use a different speed!
...
void loop() {
    ...
    //SENSORS
    unsigned char CoolantTemp[7] =        {0x03, 0x41, 0x05, rndCoolantTemp, 0xaa, 0xaa, 0xaa};  
    ...
    unsigned char AmbientAirTemp[7] =     {0x03, 0x41, 70, rndAmbientAirTemp, 0, 185, 147};
    ...
    //Data:                                             61    01    ff    ff    ff    ff
    unsigned char BMS_1[8] =              {0x10, 0x3d, 0x61, 0x01, 0xFF, 0xFF, 0xFF, 0xFF}; //FIRST FRAME
    unsigned char BMS_2[8] =              {0x30, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; //FLOW CONTROL
    //Data:                                       be    11   a0     16    f8    80    00
    unsigned char BMS_3[8] =              {0x21, 0xbe, 0x11, 0xa0, 0x16, 0xf8, 0x80, 0x00}; //CON FRAME SEQ 1
    //Data:                                       00    0f    7e    20    1c    1c    1c
    unsigned char BMS_4[8] =              {0x22, 0x00, 0x0f, 0x7e, 0x20, 0x1c, 0x1c, 0x1c}; //CON FRAME SEQ 2
    //Data:                                       1c    1c    1e    00    22    ce    02
    unsigned char BMS_5[8] =              {0x23, 0x1c, 0x1c, 0x1e, 0x00, 0x22, 0xce, 0x02}; //CON FRAME SEQ 3
    //Data:                                       ce    31    00    ff    77    00    01
    unsigned char BMS_6[8] =              {0x24, 0xce, 0x31, 0x00, 0xff, 0x77, 0x00, 0x01}; //CON FRAME SEQ 4
    //Data:                                       88    d4    00    01    88    7b    00
    unsigned char BMS_7[8] =              {0x25, 0x88, 0xd4, 0x00, 0x01, 0x88, 0x7b, 0x00}; //CON FRAME SEQ 5
    //Data:                                       00    8d    5f    00    00    89    68
    unsigned char BMS_8[8] =              {0x26, 0x00, 0x8d, 0x5f, 0x00, 0x00, 0x89, 0x68}; //CON FRAME SEQ 6
    //Data:                                       00    9d    0c    af    49    00    01
    unsigned char BMS_9[8] =              {0x27, 0x00, 0x9d, 0x0c, 0xaf, 0x49, 0x00, 0x01}; //CON FRAME SEQ 7
    //Data:                                       00    00    00    00    03    e8    00
    unsigned char BMS_10[8] =             {0x28, 0x00, 0x00, 0x00, 0x00, 0x03, 0xe8, 0x00}; //CON FRAME SEQ 7
  ...
  if (mcp2515v2.readMessage(&canMsg) == MCP2515v2::ERROR_OK) {
    ...
    Serial.print("R: ");Serial.print(canMsg.can_id);Serial.print(",");
    ...
    if(BuildMessage=="2,1,0,0,0,0,0,0,") {responseDataOBD(SupportedPID, 8);}
    if(BuildMessage=="2,1,1,0,0,0,0,0,") {responseDataOBD(MilCleared, 7);}
    
    //SEND SENSOR STATUSES
    if(BuildMessage=="2,1,5,0,0,0,0,0,") {responseDataOBD(CoolantTemp, 7);}
    if(BuildMessage=="2,1,12,0,0,0,0,0,"){responseDataOBD(rpm, 7);}
    if(BuildMessage=="2,1,13,0,0,0,0,0,"){responseDataOBD(vspeed, 7);}
    if(BuildMessage=="2,1,15,0,0,0,0,0,"){responseDataOBD(IATSensor, 7);}
    if(BuildMessage=="2,1,16,0,0,0,0,0,"){responseDataOBD(MAFSensor, 7);}
    if(BuildMessage=="2,1,70,0,0,0,0,0,"){responseDataOBD(AmbientAirTemp, 7);}
    if(BuildMessage=="2,1,60,0,0,0,0,0,"){responseDataOBD(CAT1Temp, 7);}
    if(BuildMessage=="2,1,61,0,0,0,0,0,"){responseDataOBD(CAT2Temp, 7);}
    if(BuildMessage=="2,1,62,0,0,0,0,0,"){responseDataOBD(CAT3Temp, 7);}
    if(BuildMessage=="2,1,63,0,0,0,0,0,"){responseDataOBD(CAT4Temp, 7);}
    if(BuildMessage=="2,1,148,0,0,0,0,0,"){responseDataOBD(FuelRate, 7);}
    if(BuildMessage=="2,1,19,0,0,0,0,0,"){responseDataOBD(Speed, 7);}
  ...
  if ((millis() - last_rpm) > 1000) { // change these values here to change the update frequency
    if (last_read) {
